"""Analizador CRISP-DM — Streamlit.

Sube cualquier archivo tabular y recorre las seis fases de CRISP-DM:
negocio y KPIs, comprensión de datos, preparación, modelado, evaluación y
despliegue, con un dashboard de insights en medio.
"""
...
from core import charts, deployment, insights as ins_mod, kpis as kpi_mod, loader, prep, profiling, quality
...
from core.utils import fmt_num, fmt_pct, to_datetime_series, to_numeric_series

st.set_page_config(page_title="Analizador CRISP-DM", page_icon="📊",
...
<style>
  ...
  [data-testid="stMetricValue"]{font-size:1.55rem;letter-spacing:-.01em}
  [data-testid="stMetricLabel"]{font-size:.78rem;text-transform:uppercase;letter-spacing:.05em;opacity:.75}
  .ins-card{border:1px solid rgba(128,128,128,.25);border-left:3px solid #2a78d6;
  ...
  .ins-card b{display:block;margin-bottom:.25rem;font-size:.98rem}
  .ins-card span{font-size:.9rem;opacity:.88;line-height:1.55}
...
_DEFAULTS = {
    "raw": None, "source": "", "meta": None,
    "clean": None, "prep_log": [], "plan": prep.PrepPlan(),
    "mapping": {}, "custom": [], "model": None, "selected_kpis": None,
    "currency": "$", "nav": PHASES[0][1],
}
for _k, _v in _DEFAULTS.items():
    if _k not in st.session_state:
        st.session_state[_k] = _v

# navegación diferida: los botones de "ir a…" no pueden tocar la clave del radio
# una vez instanciado, así que dejan aquí su intención y se aplica antes de pintarlo
if st.session_state.get("_goto"):
    st.session_state.nav = st.session_state.pop("_goto")


def goto(phase_key: str):
    st.session_state["_goto"] = dict(zip([p[0] for p in PHASES],
                                         [p[1] for p in PHASES]))[phase_key]
...
with st.sidebar:
    st.markdown("### 📊 Analizador CRISP-DM")
    st.caption("Metodología estándar de la industria para proyectos de datos, "
               "en seis fases.")
    ...
    if df_now is not None:
        ...
        st.caption(f"{len(df_now):,} filas × {df_now.shape[1]} columnas")
        if st.session_state.clean is not None:
            st.success("Usando datos preparados", icon="✅")
            if st.button("Volver a los datos originales", use_container_width=True):
                st.session_state.clean = None
                st.session_state.prep_log = []
                st.rerun()
        st.session_state.currency = st.text_input("Símbolo de moneda", st.session_state.currency,
                                                  max_chars=4)
    else:
        st.info("Carga un archivo para empezar.", icon="📁")
